...
This should ``behave'' exactly like $\Two^A$, but concretely, consist of sets rather than characteristic functions.
...
$f^-(b)$ (from the earlier usage) is the same as $f^-(\{b\})$.
...
We spell out the axioms for Heyting algebras in the next section, but generally speaking these are the structures that correspond to a sort of minimal version of propositional logic in which ``and'', ``or'', ``implies'', ``false'' and ``true'' interact in natural ways. 
...
\begin{lemma}
	...
	\begin{proof}
		Calculating the members of $\neg A$, it is easy to check that for every element $x\in X$, either $x\in A$ or $x\in A^*$, but not both. So $x\in A^{**}$ if and only if $x\notin A^*$ if and only if $x\in A$. 
	\end{proof}
\end{lemma}
...
\begin{laws}
	...
	\begin{tabular}{lr@{\,}l}
		\multicolumn{3}{c}{\textbf{Semilattice Laws}}\\
		...
		\textbf{Idempotency}	& $A\cap A$ & $= A$\\
								& $A\cup A$ & $= A$\\
		&&\\						
		\multicolumn{3}{c}{\textbf{Lattice Laws}}\\		
		\textbf{Absorptivity}	& $A$ & $\subseteq A\cap(B\cup A)$\\
								& $A\cup(B\cap A)$ & $\subseteq  A$\\
		\textbf{Ordering} & \multcolumn{2}{c}{$A\subseteq B$ if and only if $A=A\cap B$}\\
						& \multcolumn{2}{c}{$A\subseteq B$ if and only if $A\cup B = B$}\\
				&&\\ 
		\multicolumn{3}{c}{\textbf{Bounded Lattice Laws}}\\				
		\textbf{Identity}&$A\cap U$    &$subseteq  A$\\ 								
								&$A$ & $\subseteq A\cup \emptyset$\\
		&&\\
		\multicolumn{3}{c}{\textbf{Heyting Algebra Law}}\\								
		\textbf{Residuation} &\multicolumn{2}{c}{$A\cap B\subseteq C$ if and only if $A\subseteq B\Rightarrow C$}\\
		&&\\
		
		\multicolumn{3}{c}{\textbf{Boolean Algebra Law}}\\					
		\textbf{Double Negation} & $A^{**}$ & $\subseteq A$ \\
		&&\\
		
		\multicolumn{3}{c}{\textbf{Distributive Lattice Laws}}\\						
...
The laws we list here are not independant of one another.
In the Residuation and Double Negation laws
In particular, the two Absorption laws are equivalent when the first six laws hold.
That is, suppose the first Absoprtion law holds. Then
$A\subseteq A\cup (B\cap A)$ is always true
