...
\begin{lemma}
	For any function $\fromot fYX$, the graph $\Gamma_f$ is functional.
...
\chapter{Relations and Graphs of Functions}

\begin{defn}
A \emph{relation} between $X$ and $Y$ is characteristic function $\fromto R{X\times Y}\Two$. Of course, such a function is essentially the same as a 
subset of $X\timesY$. It is often convenient to write a relation using ``infix'' notation: $xRy$ instead of $R(x,y)$. 
\end{defn}

A relation $R$ determines a function $\fromto rX{\powerset(Y)}$ defined by the rule $x\mapsto \{y\in Y\st R(x,y)\}$. Conversely, any such function
determines a relation $R(x,y)$ if and only if $y\in r(x)$. These two correspondences are inverses of ech other.

Suppose $R\subseteq X\times Y$ and $S\subseteq Y\times Z$ are relations.
Define $R;S\subseteq X\times Z$ by $R;S(x,z)$ if and only if there exists $y\in Y$
so that $R(x,y)$ and $S(y,z)$. 

\begin{defn}
	A relation $\fromto R{X\times Y}\Two$ is \emph{total} if it is the case that for every $x\in X$, there is at least one $y\in Y$ for which $R(x,y)$. A relation is \emph{deterministic} if it is the case that for every $x\in X$, there is at most one $y\in Y$ for which $R(x,y)$.
	A relation is \emph{functional} if it is total and deterministic.
\end{defn}

Given a fucntion $\fromto fXY$, define the \emph{graph} of $f$ to be $\Gamma_f = \{(x,y)\in X\times Y\st f(x)=y\} 
...
\begin{lemma}
	...
	\begin{proof}
		If $A\subseteq \NN$ has no least element, then $A$ must be empty. This is provable by strong induction. Suppose $A\subseteq \NN$ has no least element. Then
		\begin{itemize}
			\item{}[Strong Inductive Hypothesis] Suppose that for some $k$, it is the case that for each $j<k$, $j\notin A$. 
			\item{}[Inductive Step] We claim that $k\notin A$. Otherwise, by the inductive hypothsis, all natural numbers less than $k$ do not belong to $A$. So $k$ would be the least element of $A$. 
		\end{itemize} 
		Hence for any non empty $A\subseteq \NN$, there is least element. This is the element defined by $\mu(A)$. 
	\end{proof}
...
\begin{lemma}
	...
	\begin{proof}
		...
		Suppose $A=\emptyset$, then the unique function from $A$ to $\NN$ is a monomorphism. Suppose there is a surjection $\fromto c\NN A$. Then
		for any $a\in A$, the set $c^{-1}(a)$ is not empty. So this set has a 
		smallest element. We define $\fromto mA\NN$ by $m(a) = \text{the smallest $n\in \NN$ so that $n\in c^{-1}(a)$. 
	\end{proof}
\end{lemma}
