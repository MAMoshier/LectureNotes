...
\begin{itemize}
	\item $\hat{0} \in P^0$
	\item $\suc\in P^1$
	\item for each $k<n$, $\pi^n_k\in P^n$ where $\pi^_k\in \NN^{\NN^n}$ is defined by $\pi^n_k(x_0,\dotsc, x_{n-1}) = x_k$
...
the function $\fromto {\symb{Ack}}\NN\NN$ defined by $\symb{Ack}(n) = A_n(n)$ is not. The proof is quite ingeneous. Roughly, one shows that $\symb{Ack}$ grows faster than any primitive recursive function.

To have an idea of how fast this function grows, $\symb{Ack}(0) = 1$, $\symb{Ack}(1) = 2$, $\symb{Ack}(3) = 10$, $\symb{Ack}(4) = 220$, and $\symb{Ack}(5)$ is a number far larger than the number of electrons in the visible universe. 
...
Many other constructions can be built up using the principles we have discussed. In this lecture, we discuss some of the most useful and most general.

\section{Unions}

Although not strictly needed for most applications, mathematicians generally agree that sets, no matter how they are related, can be merged into a single set with elements taken form the originals. We take this as an additional principle.
...
For a sets of sets $\{X,Y\}$, we write $X\cup Y$ instead of $\bigcup\ldots$. 
...
Co-equalizers are indeed definable using the principles we already have, but it turns out that a related notion of \emph{quotients} is more commonly used. We discuss quotients below, but build co-products first.
