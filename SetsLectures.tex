...
This lemma relates an external concept (monomorphism) to an internal one (injectivity). 
...
\begin{principle}
	For a monomorphism $\sfromto mXY$ there is a unique function $\sfromto {\kappa_m}Y\Two$ so that $m$ is an equalizer for the equation $\kappa_m(y) = \top$. In other words, if $\sfromto fWY$ is a function satisfying $\kappa_m(f(w))=\top$ for all $w\in W$, then there is a unique function $\sfromto {m\backslash f}WX$ so that $f = (m\backslash f)\circ m$.  We say that $f$ \emph{factors through $m$ uniquely}.
	...
	\[\kappa_m(y) = \begin{cases}
	    ...
	    \bot & \text{otherwise}.
	\end{cases}\]
	Suppose $\sfromto fWY$ is a function satisfying $\kappa_m(f(w)) = \top$ for all $w$. Then $(m\baskslash f)(w) = x$ if and only if $f(w)=m(x)$. 
\end{principle}
...
\begin{lemma}
	For function $X\stackrel{m}{\longrightarrow}Y\stackrel{f}{\longleftarrow}W$ where $m$ is a monomorphism, $f$ is $m$-coinvariant if and only if $\kappa_m\circ f = \top$.

\begin{proof}
	Assume $f$ is $m$-restricted, and $h\circ m = k\circ m$. For any $w\in W$, there is some $x\in X$ so that $f(w)=m(x)$. Hence $h(f(w)) = h(m(x)) = k(m(x)) = k(f(w))$. 
	Since this works for any $w$, $h\circ f=k\circ f$. 
	
	Assume $f$ is not $m$-restricted. So there is some $w\in W$ for which $f(w)\neq m(x)$ for all $x\in X$. Define functions $Y\doublerightarrow{h}{k}\{a,b,c\}$ by 
...
\begin{principle}
	For functions $X\stackrel{m}{\longrightarrow}Y\stackrel{f}{\longleftarrow}W$, if $m$ is a monomorphism and $f$ is $m$-coinvariant, then there is a unique function $\fromto {m\backslash f}WX$ so that $f=m\circ (m\backslash f)$.
	
	The function $m\backslash f$ is defined by $w\mapsto x$ if and only if $f(w)=m(x)$.
\end{principle}
