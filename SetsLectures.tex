...
That is, $f^+(\{x\}) = \{f(x)\}$. Second, $f^_$ preserves all unions So $f^+(\bigcup_{i\in I}A_i) = \bigcup_{i\in I}f^+(A_i)$. Not that $f$ does not necessarily preserve intersections.
...
As simple as this result is, this is valuable because it provides an internal characterization (injections deal with elements) for an external property (monomorphisms deal with composition). Epimorphisms are also defined externally, but we do not yet have an internal characterization. Taking a closer look at injectivity gives a clue about how we might characterize epimorphisms internally, too.
