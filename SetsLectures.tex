...
\section{The Basic Structure of $\powerset(A)$}
...
Suppose $\fromto fAB$ is a function and $X\subseteq A$. Then the \emph{forward image} if $X$ is the set $\{y\in B\st \text{$f(x)=y$ for some $x\in X$}\}$.
\begin{defn}
	A \emph{complete semilattice} is a function $\fromto \bigvee{\powerset(C)}C$
	satisfying $\bigvee\circ \{-\} = \id_S$ and $\bigvee(\bigvee\mathcal X)$
\end{defn}
