...
\begin{lemma}
	...
	\begin{proof}
		For $\fromto fXY$, let $Z = f^+(X)\subseteq Y$. Let $m$ be the inclusion map from $f^+(X)$ to $Y$. We already remarked that any inclusion is a monomorphism. Let $e$ be the corestriction of $f$ to $f^+(X)$. Clearly, for any $y\in f^+(X)$, there is at least one $x\in X$ so that $f(x)=y$. So $e$ is an onto function, hence is an epimorphism.
	\end{proof}
\end{lemma}
