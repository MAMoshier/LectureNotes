...
There is little sense in saying that $\rank$ and $\suit$ are ``operations'', but they are functions \emph{from} $\symb{Deck}$ to $\symb{Rank}$ and $\symb{Deck}$ to $\symb{Suit}$, respectively.

The functions $\rank$ and $\suit$ capture some structure of the elements of $\symb{Deck}$. In particular, for any rank $r$ and any suit $s$, there is exactly one card $c$ so that $\rank(c)=r$ and $\suit(c)=s$. 
...
\begin{signature}\label{sig:SetSignature}
  ...
  For any thing $x$, either $x$ \emph{is in} $X$ or $x$ \emph{is not in} $X$. 
  We write $x\in A$ if $x$ is in $X$ and $x\notin X$ if $x$ is not in $X$.
...
of set theory. 
We do not try to justify that claim explicitly for now.
...
$X$ and $Y$ must be equal. So we state this as another axiom.
...
Notice that $g\circ f$ is only defined when the \emph{domain} of $g$ matches the \emph{codomain} of $f$.
...
\end{tikzpicture}
\]
We do not really need to draw $g\circ f$ as a separate arrow because the \emph{path} from $X$ to $Y$ to $Z$ is already implicitly a depiction of $g\circ f$.
...
\[
  ...
  \end{tikzpicture}
\]
Since $\One$ has only a single element, it can ``point'' only to a single element of $X$.
So we might refer to a function $\One\to X$ as a \emph{pointer} into $X$.
...
In effect, this principle simply asserts that elements of a set $X$ and functions $\One\to X$ are interchangible: from $a\in X$ we get $\fromto {\hat{a}}\One X$; from $\fromto p\One X$.
...
Then $f(a)=g(a)$ is true for every $a\in X$ because $\bullet$ is the only possible value: $f(a) = \bullet = g(a)$. 
...
$x\mapsto \hat{b}(\Diamond_X(x))$. But since $\hat{b}(\Diamond_X(x))= \hat{b}(\bullet) = b$, the rule simplifies to $x\mapsto b$. 
In short, this is the function sending any element of $X$ to the constant $b$. 
...
\begin{exercises}
	\begin{firstexercise}
		...
		\item Suppose that $\fromto fXY$ is a function. Show that for every $a\in A$, $\widehat{f(a)} = f\circ \hat{a}$.
	\end{firstexercise}
\end{exercises}
...
for each $x\in\emptyset$, $f(x)=g(x)$ because there are no $x$'s to concern us. Hence by Principle \ref{ax:FunctionExtensionality}, $f=g$. The empty ``rule'' that tells us to do nothing actually specifies a function from $\emptyset$ to $X$. So for any set $X$, there is exactly one function from $\emptyset$ to $X$. Let's make that official.
...
First, we can form a subset of $X$ by specifying an equation $f(x)=g(x)$ for any two functions $X\doublerightarrow{f}{g}Y$, and picking out the particular solutions.
...
\begin{defn}\label{def:inverse-image}
	For a function $\sfromto fXY$, and a value $c\in Y$, 
	\[f^-(c) = \{x\in X\st f(x)=c\}.\] 
	In this case, $f^-(c)$ is called the \emph{inverse image of $c$ with respect to $f$}.
\end{defn}

A diagram can help understand inverse images as well. Suppose $\fromto fXY$ and $c\in C$,
...
\end{tikzpicture}
\]
For any other function $\sfromto gWX$ that makes following diagram commute:
\[
\begin{tikzpicture}
\matrix (m) [row sep=3em,column sep=4em,minimum width=2em]
{
...
A product of $X$ and $Y$ is depicted as a diagram
	\[
	\begin{tikzpicture}
	\matrix (m) [row sep=3em,column sep=4em,minimum width=2em]
	{
...
We can now form a single function from $X\times A$ to $Y\times B$ by combining $f$ and $g$ ``independently''. 
...
For example, the function $s\fromto f\RR\RR$ defined by the rule $f(x) = \sin(x + c)$ depends on the constant $c$.
There is a related function $\sfromto g{\RR\times \RR}\RR$ defined by $g(c,x) = \sin(x+c)$.
Though $g$ describes the same behavior as $f$, it makes the parameter explicit as another argument. It will be helpful to relate $g$ -- a function on pairs -- to $f$ a function with a parameter.
This leads to the following definition.

\begin{defn}
	For sets $X$ and $Y$, a \emph{parametric function from $X$ to $Y$} is a function $\sfromto g{P\times X}Y$ for some set $P$.
	The set $P$ may be called the \emph{set of parameters}.
	
	Suppose $\sfromto f{Q\times X}Y$ is a parametric function with parameter set $Q$ and $\sfromto kPQ$ is a function.
	Then we can form another parametric function with parameters in $P$ by composing: $f\circ (k\times \id_A)$.
	The function $k$ acts lie a \emph{change of parameters} because it transforms the parametric function with parameters in $Q$ into a parametric function with parameters in $Q$. 
	Specifically, $f\circ(k\times \id_X)$ is given by the rule $(c,a)\mapsto f(k(c),a)$.
	
	An \emph{evaluation map} for $X$ and $Y$ is a parametric function $\sfromto a{F\times X}Y$, so that for any 
...
\begin{principle}
	...
	Calculating how $\sfromto {\curry[f]}P{Y^X}$ must behave, we see that for any parameter $p\in P$, $\curry[f](p)$ is the function from $X$ to $Y$ given by the rule $x\mapsto f(p,x)$.
	So for any $p\in P$, $\curry[f](p)$ is the function such that for any $x\in X$, $\curry[f](p)(x) = f(p,x)$. 
\end{principle}
...
So it would be convienent to have a notation that permits us to describe the behavior of a function without giving the function a name. 
The logician Alonzo Church \cite{church} was interested in the fundamental idea of just what \emph{is} a function. He proposed a notation for describing functions. 
...
This ``$\lambda$'' notation is widely adopted in computer science. Indeed, it appears even in languages such as Python.  
...
\begin{example}
	\begin{itemize}
	\item For $\sfromto fXY$ and $\sfromto gYZ$, the composite function $g\circ f\in Z^X$ is $\lambda x.g(f(x))$.
...
Suppose $S$ is a set modelling the students at Chapman and $M$ is a et modelling the academic majors the university offers: $\symb{Mathematics}$, $\symb{Philosophy}$, $\symb{HeadScratching}$, and so on. Then elements of $S$ (students) can be related to elements of $M$ (majors) by `is majoring in' as in
``$\symb{Jethro}$ is majoring in $\symb{Biology}$.'' Because a student might have a double major, we can not model the situation as a function, at least not in the most obvious way. Instead, we introduce the notion of a \emph{(binary) relation}. The same idea, generalized to higher dimensional relations, is at the heart of what we call \emph{relational databases}.  
We will investigate the general concept of a \emph{relation} in more detail later. For now, we need them primarily as they relate to functions. 

\begin{defn}
	A \emph{binary relation from $X$ to $Y$} is subset $R\subseteq X\times Y$.
	A \emph{binary relation on $X$} is a relation from $X$ to $X$. Since we will only be concerned with binary relations, from now on we refer them simply as \emph{relations}
	
	For a relation $R$ from $X$ to $Y$, we will say ``$x$ is $R$-related to $y$'' and write $R(x,y)$ when $(x,y)\in R$. In many situations, we use ``infix'' notation, writing $x\mathrel{R}y$ instead of $R(x,y)$.
...
\begin{example}
	The relation of ``less than or equal to'' $\leq$ on real numbers can be regarded as a subset $\mathord{\leq}\subseteq {\RR\times\RR}$ defined by $(x,y) \in \mathord\leq$ when $x$ is actually less than or equal to $y$ and $(x,y)\notin \mathord\leq$ otherwise. 
...
\begin{defn}
Suppose $R\subseteq X\times Y$ and $S\subseteq Y\times Z$ are relations. Define $R;S\subseteq X\times Z$ by $x\mathrel{R;S}z$ if and only if there is some $y\in Y$ so that $x\mathrel{R}y$ and $y\mathrel{S}z$.

For set $X$, let $\Delta_X\subseteq X\times X$ be defined as $\Gamma_{\id_X}$. That is, $x\mathrel{\Delta_X}y$ if and only if $x=y$.
\end{defn}
...
\begin{exercises}
\begin{firstexercise}
	\item Define addition $\sfromto {+}{\NN\times \NN}\NN$ by primitive recursion. That is, find functions $\sfromto b\NN\NN$ and and $\sfromto r{\NN\times\NN\times \NN}\NN$ so that
	\begin{align*}
		m+0 &= b(m)\\
		m+n^\nxt &= r(m,n,m+n)
	\end{align*}
...
It is a theorem (that we can not prove here) that there is a technical sense in which primitive recursion is implementable, say in Python, by nested ``for'' loops. To get a taste of what this means, let us pretend that $\sfromto bPX$ and $\fromto r{P\times \NN\times X}X$ are somehow defined by Python functions. Then the following code implements $\primrec[b,r]$.
...
One answer is that programming languages are not merely for computing functions. They are used to implement lots of behavior that is not so easily cast in terms of sets and functinos. Another answer, though, is internal to set theory. Namely, there are recursive functions on $\NN$ which are not primitive recursive.
...
\begin{itemize}
	...
	\item If $g\in P^n$ and for each $k<n$, $f_i\in P^m$, then $g\circ \langle f_0,\dotsc, f_{n-1}\rangle \in P^m$
...
\begin{align*}
A_0 & \suc\\
A_{k^\nxt} = \primrec[\hat{A_k(k)}, A_k\circ \pi^2_1]
\end{align*}
...
For a set $A$, the lists consisting of elements from $A$ also form a set.
The structure of this set is similar to $\NN$.

\begin{defn}
	For a set $A$, a \emph{simple $A$-recurrrence} is a set with two functions
	$\One \stackrel{\hat b}{\longrightarrow} X \stackrel{r}{\longleftarrow} A\times X$.
	
	For a set $A$, an \emph{$A$-list set} is a simple $A$-recurrence 
	$\One \stackrel{\hat{e}}{\longrightarrow} L \stackrel{c}{\longleftarrow} A\times L$ so that for any simple $A$-recurrence $\One \stackrel{\hat b}{\longrightarrow} X \stackrel{r}{\longleftarrow} A\times X$ there is exactly one function $\fromto hLX$ so that 
	\begin{align*}
		h\circ \hat e 	&= \hat b\\
...
\begin{principle}
	The collection of lists with items drawn from $A$ is a set, denote by $\List[A]$.
	The functions $\One\stackrel{\widehat{[\,]}}{\longrightarrow}\List[A]\stackrel{\mathord{:}}{\longleftarrow}A\times \List[A]$
	constitute an $A$-list set, where the function $\sfromto {mathord{\colon}}{A\times \List[A]}{\List[A]}$ is the function given by the rule $(a,L)\mapsto a:L$.
	
	For $\One \stackrel{\hat b}{\longrightarrow} X \stackrel{r}{\longleftarrow} A\times X$, we reuse our notation for recursive functions defined from $\NN$
	and write $\srec[\hat b,r]$ for the unique function 
	satisfying 
	\begin{align*} 
	 \srec[\hat b,r]([\,]) &= b\\
	 \srec[\hat b,r](a:L)  &= r(a, \srec[\hat b, r](L))
	\end{align*}
\end{principle}
...
\begin{exercises}
	\begin{nextexercise}
		...
		\item Write a scheme for ``primitive $A$ recursion'', specified by functions $P\stackrel{b}{\longrightarrow}X\stackrel{r}{\longleftarrow}P\times \List[A]\times X$. Write the equations involving $b$ and $r$ that should determine a unique function $P\times \List[A]\to X$. 
...
\begin{principle}\label{ax:powerset}
	For any set $X$, the collection of subsets of $X$ is a set, denoted by $\powerset(X)$, called the \emph{power set of $X$}. Moreover, there is a function $\fromto {\ni_X}{\powerset(X)\times X}\Two$ defined by 
...
\begin{lemma}
	...
	\begin{proof}
		The proof of this is fairly technical and not especially infomrative. A sketch will suffice.
...
\begin{lemma}
	...
	\begin{proof}
		$x\in f^{-}(\bigcup_{i\in I}B_i)$ if and only if $f(x)\in \bigcup_{i\in I}B_i$ if and only if $f(x)\in B_k$ for some $k\in I$ if and only if $x\in f^{-}(B_k)$ for some $k\in I$ if and only if $x\in \bigcup_{i\in I}f^{-}(B_i)$. The proof for $\bigcap$ is similar with ``for some \ldots'' replaced by ``for all \ldots".  
	\end{proof}
\end{lemma}
...
\begin{lemma}
	...
	\begin{proof}
		...
		By definition, this means $x\in f^-(B)$. This shows that $A\subseteq f^-(B)$. Conversely, suppose $A\subseteq f^-(B)$. For $y\in f^+(A)$, there is some $x\in A$ so that $f(x)=y$.
		So there is some $x\in f^-(B)$ so that $f(x)=y$. Thus $y=f(x)\in B$. This shows that $f^+(A)\subseteq B$
	\end{proof}
\end{lemma}
...
Although not strictly needed for most applications, mathematicians generally agree that sets, no matter how they are related, can be merged into a single set with elements taken form the originals. That is, union ($\bigcup$) is meaningful for any set of sets. We take this as an additional principle.
...
\begin{defn}
	For a set $X$, a \emph{partition of $X$} is a set $P\subseteq\powerset(X)$ so that $\bigcup_{A\in P}A = X$ and for every $A,B\in P$, if $A\neq B$ then $A\cap B=\emptyset$. The sets in $P$ are called \emph{blocks}. 
	...
	That is, $X/E$ is the range of the function $[\cdot]_E$.
\end{defn}
...
This relation is total because every block $B\in X/E$ non-empty. 
So there is some $x\in B$, and thus $B\mathrel{F}f(x)$.
The relation $F$ is also deterministic because if $B\mathrel{F}y$ and $B\mathrel{F}y'$, then there is some $x\in B$ for which $f(x)=y$
and there is some $x'\in B$ for which $f(x')=y'$. 
But $x,x'\in B$ implies $x\mathrel{E}x'$.
Hence $f(x)=f(x')$.

\begin{defn}
	Suppose $E\subseteq X\times X$ is an equivalence relation. A function $\fromto fXY$ is \emph{$E$-invariant} if $x\mathrel{E}x'$ implies $f(x)=f(x')$.
...
\section{The Axioms of Choice and Replacement}
...
One such axiom is called the Axiom of Choice. Roughly is says that some how, one make arbitrarily many choices all at once. A famous example, due to Russel, is having an infinite collection of identical socks. One needs the Axiom of Choice to say that the socks can be put into pairs. The Axiom of Choice is quite useful in mathematics, sometimes to prove something that genuinely needs the axoim, other times to simplify a proof that could have been proved without it. Because of the special role the Axiom of Choice plays in mathematics, we return to discuss the Axiom of Choice in detail after we have some other concepts in place.

Another axiom says, roughly, that if a collection is the same size as a set, then it is a set. This is the Axiom of Replacement (think of replacing the elements of a set with elements of the collection, thus forming a new set). This is an important axiom for dealing with set theory, \emph{per se}, but it does not come up much in practice. We can state a form of the axiom that is technical, but occasionally useful.

\begin{principle}
	Suppose $I$ is a set, and for each $x\in I$, $a_i$ is a mathematical entity. Then there is a set, denoted by $\{a_i\}_{i\in I}$,satisfying $x\in \{a_i\}_{i\in I}$ if and only if $x=a_i$ for some $i\in I$. The set $\{a_i\}_{i\in I}$ is sometimes called an \emph{indexed set},
	or \emph{indexed family}.
\end{principle}

For example, let $V_i = \NN$ and let $V_{k+1} = \powerset(V_k)$. Then the principle tells us that $\{V_i\}_{i\in \NN}$ is a set.
Now define $V_\omega = \bigcup_{i\in \NN}V_i$. This is a set that can not be constructed without Replacement. This princple can be made formal by introducing some additional logical machinery, but we will not make use of it in these lectures enough to warrant that attention.
