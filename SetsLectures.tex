...
More specifically, $\powerset(U)$ is a \emph{complete atomic Boolean algebra}.
First, completeness we need the following definition:
\begin{defn}
	Suppose $X$ is a set and $\leq$ is a reflexive, transitive, anti-symmetric relation on $X$. [Think of $\powerset(U)$ together with $\subseteq$]. 
	Let $\fromto aIX$ be a function. Writing $a_i$,
	instead of $a(i)$, then we refer to $a$ as an \emph{(indexed) family of elements of $X$}, and write it as $\{a_i\}_{i\in I}$.
	
	An \emph{upper bound} of a family $\{a_i\}_{i\in I}$ is an element $x\in X$
	so that $a_k\leq x$ holds for each $k\in I$. A \emph{least upper bound} of the family, written as $\bigvee_{i\in I}a_i$ is an element of $X$ so that
	\begin{itemize}
		\item $\bigvee_{i\in I}a_i$ is an upper bound of $\{a_i\}_{i\in I}$; and 
		\item if $c$ is an upper bound of $\{a_i\}_{i\in }$, then $\bigvee_{i\in I}a_i\leq c$. 
	\end{itemize}
\end{defn}
...
We have introduced the notation $\{x\in A\st f(x)=g(x)\}$ to mean the subset of $A$ consisting of the particular solutions to the equation $f(x)=g(x)$. 
We have also slightly extended this for inverse images of elements: $\{x\in A \st f(x)=b\}$. 
...
Suppose $\fromto fUV$ is a function and $B\subseteq V$. Then $\{x\in U\st f(x)\in B\}$ defines a subset of $U$.

Suppose $\fromto fAB$ and $\fromto gCB$ are functions, then $\{(x,y)\in A\times B\st f(x)=g(y)\}$ defines a subset of $A\times B$.

If $\{x\in A\st P(x)\}$ defines a subset of $A$, then $\{(x,y)\in A\times B\st P(x)\}$ defines a subset of $A\times B$. 

If $\{x\in A\st P(x)\}$ and $\{x\in A\st Q(x)\}$ define subsets of $A$,
then $\{x\in A\st P(x)\wedge Q(x)\}$.

If $\{x\in A\st P(x)\}$ and $\{x\in A\st Q(x)\}$ define subsets of $A$,
then $\{x\in A\st P(x)\to Q(x)\}$.

If $\{x\in A\st P(x)\}$ and $\{x\in A\st Q(x)\}$ define subsets of $A$,
then $\{x\in A\st P(x)\vee Q(x)\}$.

$\{x\in A\st \ff \}$ defines a subset of $A$.

$\{x\in A\st \tt\}$ defines a subset of $A$.

If $\{(x,y)\in A\times B\st P(x,y)\}$ defines a subset of $A\times B$,
then $\{y\in B \st \exists x\in A.P(x,y)\}$ defines a subset of $B$.

If $\{(x,y)\in A\times B\st P(x,y)\}$ defined a subset of $A\times B$,
then $\{y\in B \st \forall x\in A.P(x,y)\}$ defines a subset of $B$.
...
We have noted that for any subset $A\subseteq U$, 
$\{x\in U\st \kappa_A(x)=\top\} = A$. 

Suppose $\fromto f{X\times Y}\Two$ is a function, then so is $\exists x.
