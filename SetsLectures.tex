...
\begin{lemma}
	...
	\begin{proof}
		The proof of this is fairly technical and, for now, is beyond the scope of this discussion. Concretely, $E$ consists of those $x\in X$ so that $R(w,x) = \top$ for some $w\in W$; $A$ consists of those $x\in X$ so that $R(w,x)=\top$ for all $w\in W$.
	\end{proof}
\end{lemma}
		as $\pi_1^{-1}(A)$.  
