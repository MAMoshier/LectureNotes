...
to subsets of $A$, and vice versa. It makes sense also to suppose that the actual collection of subsets of a set $A$ form a set, called the \emph{powerset of $A$}.
...
The fact that two of them use English is not practical problem, as long as what we write is unambiguous. On the other hand, $\{x\in \RR\st \text{$x$ is nearly $0$}\}$
is probably not a good specification (unless we make the meaning of ``nearly $0$'' precise). So in practical situations, it is harmless to write clear unambiguous plain language as part of a specification. Nevertheless, it is also helpful to have in mind some formal aspects of this notation.
...
For starters, we define certain operations on $\Two$.

\begin{defn}
	Define the following functions:
	\begin{itemize}
		\item $\fromto {\mathord{wedge}}{\Two\times\Two}\Two$ to be the characteristic function of the subset $\{(\top,\top)\}\subseteq \Two\times \Two$,
		\item $\fromto {\mathord{\vee}}{\Two\times \Two}\Two$ to be he characteristic function of the subset $\{(\bot,\top), (\top,\bot),(\top\top)\}\subseteq\Two\times \Two$,
		\item $\fromto {\mathord{\neg}}\Two\Two$ to be the characteristic function of the subset $\{\bot\}\subseteq\Two$,
	\end{itemize}
\end{defn}
