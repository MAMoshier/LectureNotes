...
Third, for any solution $s$, the function into the set of particular solutions is defined
by the same rule as $s$.
...
\begin{defn}\label{def:subset-classifier}
	A \emph{subset classifier} is a set $S$ with a distinguished element $t\in S$ so that for any set $A$ and any subset $B\subseteq A$, there is exactly one function $\fromto kTA$ for which $B = k^{-1}(t)$.
...
Just as Principle \ref{ax:pointers} asserts that elements of $A$ and functions $\One\to A$
are interchangeable, Principle \ref{ax:subset-char} asserts that the subsets of $A$ and 
the functions $A\to\Two$ are interchangeable.
...
\begin{exercises}
	\begin{enumerate}
		...
		\item Suppose $A$ is a finite set with $m$ elements and $B$ is a finite set with $n$ elements. How many elements are in $A\times B$? Describe in plain English why it makes sense to refer to $A\times B$ as a ``product.''
	\end{enumerate}
\end{exercises}

\section{Function Sets, Partial Evaluation}
...
There is a related function $\fromto g{\RR\times \RR}\RR$ defined by $g(c,x) = \sin(x+c)$. Evidently, $g$ describes the same behavior as $f$, but it makes the parameter explicit. This leads to the following definition.
...
\begin{defn}
	...
	We may refer to $k$ as a \emph{change of parameters} function because $k$ transforms
...
\begin{axiom}
	...
	Calculating how $\fromto {\curry[f]}C{B^A}$ must behave, we see that $\curry[f](c)$
	is the function from $A$ to $B$ given by the rule $x\mapsto f(c,x)$. So for any $c\in C$
	and $a\in A$, $\curry[f](c)(a) = f(c,a)$. 
\end{axiom}
...
\begin{principle}\label{ax:powerset}
	...
	For a function $\fromto f{C\times A}\Two$, then the rule $x\mapsto \{a\in A\st f(c,a)=\top\}$ determines a function from $C$ to $\powerset(A)$. 
\end{principle}

This principle means that any function $\fromto k{C\times A}\Two$ determines a function $\fromto hC{\powerset(A)}$ satisfying $a\in h(c)$ if and only if $k(c,a)=\top$.
But this same $k$ also determines a subset of $C\times A$ by $k^{-1}(\top)$.
So evidently functions $C\to\powerset(A)$, functions $(C\times A)\to \Two$, functions $C\to \Two^A$ and subsets of $C\times A$ are all interchangeable.
In Lecture \ref{lec:relations} we will discuss this interchange more thoroughly.
...
\begin{defn}
	...
	\begin{itemize}
		\item $g$ is an \emph{injection} if it is the case that $g(x)=g(y)$ implies $x=y$ for every $x,y\in A$;
...
Suppose $\fromto gBC$ is a monomorphism. Then it clearly is also an injection. 
That is, suppose $g$ is a monomorphism and that $g(b_0)=g(b_1)$. We already know
that $\widehat{g(b_0)} = g\circ \widehat{b_0}$. So $g\circ \widehat{b_0} = g\circ \widehat{b_1}$. Because $g$ cancels on the left, $\widehat{b_0} = \widehat{b_1}$.
But this implies that $b_0=b_1$. The converse of this is also true.

\begin{lemma}
	A function $\fromto gBC$ is a monomorphism if and only if it is an injection.
	\begin{proof}
		The previous paragraph proves that every monomorphism is an injection. To prove the converse, assume that $g$ is an injection. That is, $g(b_0)=g(b_1)$ implies $b_0=b_1$.
		For two functions $\fromto {f_0,f_1}AB$, suppose $g\circ f_0=g\circ f_1$. For any $x\inA$, then $g(f_0(x)) = g(f_1(x))$. So $f_0(x)=f_1(x)$. Hence by the Principle of Function Extensionality, $f_0=f_1$.
	\end{proof}
\end{lemma}
