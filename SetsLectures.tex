...
A basic class of monomorphisms consists of the inclusion maps of subsets. That is,
if $A\subseteq X$, then the inclusion function $\fromto iAX$ is obviously one-to-one, so it is a monomorphism. 

Recall that for any function $\fromto fXY$, we may define the subset of $Y$
which is the forward image of $X$ via $f$. That is, $f^+(X) = \{y\in Y\st \exists x\in X.f(x)=y\}$. We had an additional principle that $f$ itself co-restricts to $f^+(X)$. That is, the rule defining $f$ also determines a function from $X$ to
$f^+(X)$. Evidently, inclusion functions are monomorphic. And $f$ corestricted to 
...
Suppose $\fromto fXY$ is an isomorphism.
It is easy to check that it is both a monomorphism and an epimorphism. 
That is, if $f\circ g_0 = g\circ g_1$, then \[g_0 = f^{-1}\circ f\circ g_0 = f^{-1}\circ f\circ g_1 = g_1.\] 
So $g$ is a monomorphism.
The proof that it is an epimorphism is left as a (very easy) exercise. 
Since we know that monomorphisms are the same things as one-to-one functions and epimorphisms are the same things as onto functions, we now know that isomorphisms are one-to-one, onto functions, i.e., ar bijections. This confirms the idea that two sets that are ``essentially the same'' differ by a systematic renaming of elements.

The converse is also true: every bijection is an isomorphism.
The proof is slightly more technical, requiring some additional effort.

\begin{lemma}
	Every monomorphism is an equalizer.
	
	\begin{proof}
		For monomorphism $\fromto fAB$ 
	\end{proof}
Suppose $\fromto fAB$ is a monomorphism and an epimorphism. The characteristic 
function of $f^+(A)$ is the constant function $y\mapsto \top$. That is, $f$ is itself an equalizer of the equation $\kappa_B(y) = \top$. 

ideas that we can develop here.
...
Suppose $G\subseteq X\times Y$ (with inclusion $i$) has the property that for each $x\in X$, there is a unique $y\in Y$ so that $(x,y)\in G$. the composition $\pi_0\circ i$ is an isomorphism with inverse $h$.
