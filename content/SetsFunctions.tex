\chapter{Sets}\label{lec:sets}

\begin{goals}
\begin{goal}{Lecture}
\item Describe the basic structure of sets.
...
\emph{Sets} are the mathematician's way of thinking about \emph{collections} of objects. Mathematical examples will be the set of natural numbers, the set of pairs of natural numbers, the set of integers, and so on.
...
\emph{Functions}, introduced formally in Lecture \ref{lec:functions} are the mathematicians way of thinking about attributes of the things in a collection, like ``the color of'', ``the mass of'', ``the location of'', ``the father of'', ``the favorite book of the person to the left of'' and so on.
...
\end{itemize}
The answers to these questions lead to some basic principles governing the ways that mathematicians actually use sets and functions.
We could be more formal and present these principles as \emph{axioms}, but the word ``axiom'' is used in mathematics to suggest that we intend the principles to be formal and to express everything in a minimal way.  We do not need either the formality or the minimality here.
Nevertheless, everything we say in these lectures could be presented that way.  

\section{Set Basics}\label{sec:SetBasics}

A set consists of things that are ``in'' the set. All other things are ``not in'' the set. In our running example, $A\spadesuit$ is in the set $\symb{Deck}$, but $25$ is not in $\symb{Deck}$. Let us make the idea precise.

\begin{principle}[Basic Structure of Sets]\label{sig:SetSignature}
  A \newterm{set} is a mathematical entity $X$ with the following feature. 
  For any mathematical entity $x$, either $x$ \newterm{is in} $X$ or $x$ \newterm{is not in} $X$. 
...
\begin{principle}[Finite Sets]\label{princ:FiniteSets}
...
We do not try to justify anything for now, but introduce them informally along with the standard symbols used to denote them.
...
\begin{defn}
  For sets $X$ and $Y$, we say that \newterm{$X$ is a subset of $Y$} provided that every element of $X$ is an element of $Y$. We 
  write this as $X\subseteq Y$, and say that \newterm{$X$ is included in $Y$}.  We may also write $Y\supseteq X$ 
  to mean the same thing, and say that \newterm{$Y$ is a superset of $B$}.

  If $X$ is \emph{not} a subset of $Y$, we write $X\not\subseteq Y$. If $X\subseteq Y$ and $Y\not\subseteq X$, then $X$ is called a
  \newterm{proper subset of $Y$}. To indicate that $X$ is a proper subset of $Y$, we may write $X\subsetneq Y$. Some people write $X\subset Y$ for proper subsets, but we will never use that symbol in this course.
\end{defn}
...
\begin{principle}[Set Extensionality]\label{ax:SetExtensionality}
...
Draw depictions of the following sets
  ...
  \item $\{\ff,\tt$
  \item $\{\bullet\}$
  \item $\{\tt,\ff,3,5,1, \bullet\}$
  \end{exercise}
\end{exer}

\chapter{Functions}\label{lec:functions}
...
\begin{principle}[Basic Structure of Functions]\label{ax:FunctionSignature}
	\begin{itemize}
	\item For a set $X$ and a set $Y$, there are things called \newterm{functions from $X$ to $Y$}. We write $\fromto fXY$ or $A\stackrel{f}{\to} B$ to indicate that $f$ is a function from $X$ to $Y$. 

	\item For $\fromto fXY$, the set $X$ is called the \newterm{domain} of $f$ and $Y$ is called the \newterm{codomain} of $f$.
	
	\item For any function $\fromto fXY$ and any element $a\in X$, $f$ and $a$ determine an element of $Y$, written $f(a)$, and read ``$f$ of $a$''.
	\end{itemize}
\end{principle}

A function may sometimes also be called a \newterm{map}, a \newterm{transformation}, or an \newterm{operation}. 
As we will see, however, \newterm{operation} is somewhat misleading, so we usually avoid it. 

Often, a function $\fromto fXY$ is \emph{defined} by a rule. 
...
So we may define the same function $f$ by saying that \newterm{$f$ is given by the rule}
	\[x\mapsto x^2 + 4x + 2.\]
...
\begin{principle}[Identities and Function Composition]\label{princ:CategoryOfFunctions}
	
	\begin{itemize}
		\item For any set $X$, there is a function $\fromto {\id_X}XX$ defined by the rule $x\mapsto x$.
		This is called the \newterm{identity} function on $A$.
		...
		This is called the \newterm{composition of $g$ and $f$} (or sometimes \newterm{$g$ following $f$}).
	\end{itemize}
\end{principle}
...
\begin{exer}
	...
	\begin{exercise}
		\item Depict four different functions from the set $\{1,2,3\}$ to the set $\{\ff,\tt\}$. [Draw four different diagrams.]
  ...
  \begin{multicols}{2}
    ...
    \item \begin{tikzpicture}[ scale=.75, >=stealth, bullet/.style={
	  ...
	  \end{enumerate}
	  \end{multicols}
		\item Let $A = \{1,2,3\}$. Let $B=\{a,b,c,e\}$ and let $C = \{\ff,\tt\}$.
...
\begin{principle}[Equality of Functions]\label{FunctionExtensionality}
...
\begin{defn}
	Let $\bullet$, $\ff$ and $\tt$ be fixed symbols. Then define
	\begin{align*}
		\One &= \{\bullet\}\\
		\Two &= \{\ff,\tt\}
	\end{align*} 
	The single element of $\One$ is intended to look like a generic point in an internal diagram.
	The element $\tt$ is meant to indicate `True' and $\ff$, 'False'.
\end{defn}
...
So we might refer to a function $\sfromto p\One X$ as a \newterm{pointer} into $X$.
...
\begin{principle}[Elements Determine Pointers]\label{princ:Pointers}
...
\begin{defn}\label{def:terminal}
	A set $T$ is \newterm{terminal} if it is the case that for any set $X$ there is exactly one function from $X$ to $T$.
\end{defn}

\begin{principle}[A Terminal Set Exists]\label{princ:TerminalSet}
...
\begin{defn}
	An \newterm{initial set} is a set $I$ so that for any set $X$, there is exactly one function from $I$ to $X$.
\end{defn}

\begin{principle}[An Initial Set Exists]\label{princ:InitialSet}
...
Let us call such a value a \newterm{particular solution to the equation $f(x)=g(x)$}. 
...
\begin{defn}\label{def:equalizer} 
	For two functions $X\doublerightarrow{f}{g} Y$, a \newterm{solution} is a function $\sfromto sSX$ so that 
	$f\circ s = g\circ s$.
	Thus for example, if $a\in A$ is a particular solution then the pointer $\hat a$ is a solution.

	For functions $A\doublerightarrow{f}{g} B$, an \newterm{equalizer} is a solution $\sfromto eEX$ so that for any solution $\sfromto sSX$, there is exactly one function $\sfromto hSE$ so that $e\circ h = k$.
\end{defn}

\begin{principle}[Equalizers Exist]\label{princ:Equalizers}
	...
	The function $\sfromto i{\{x\in X\st f(x)=g(x)\}}A$ given by the rule $x\mapsto x$ (called an \newterm{inclusion map}) is an equalizer for $f$ and $g$.
...
\begin{defn}\label{def:inverse-image}
	...
	\[f^-(c) = \{x\in X\st f(x)=c\}.\] 
	In this case, $f^-(c)$ is called the \newterm{inverse image of $c$ with respect to $f$}.
\end{defn}
...
\begin{defn}\label{def:subset-classifier}
	...
	A \newterm{pointed set} is a set $P$ with a distinguished element $p\in P$.
	
	A \newterm{subset classifier} is a set $T$ with a distinguished element $t\in T$ so that for any set $X$ and any subset $A\subseteq X$, there is exactly one function $\fromto kXT$ for which $A = k^-(t)$.
	That is, $A$ is uniquely defined as the inverse image of $t$ with respect to a function into $T$.
\end{defn}

\begin{principle}[$\Two$ and Characteristic Functions]\label{ax:subset-char}
	
	The set $\Two$ with the distinguished element $\tt$ is a subset classifier.
	For subset $A\subseteq X$, the function corresponding to $A$, called the \newterm{characteristic function of $A$}, is denoted by $\kappa_A$.
	In other words, $\kappa_A$ is the unique function for which $A = \kappa_A^-(\tt)$.
...
What makes this work is that points in the plane \newterm{project} onto the two axes in a universal way.
Products of sets generalize this idea.

\begin{defn}
For sets $X$ and $Y$, a \newterm{table} consists of two functions $X\stackrel{f}{\longleftarrow} T\stackrel{g}{\longrightarrow}Y$.
Note that the two functions have the same domain. We may call the two functions \newterm{legs} of the table.

For sets $X$ and $Y$, a \newterm{product of $X$ and $Y$} is a table $X\stackrel{p}{\longleftarrow} P\stackrel{q}{\longrightarrow}Y$ so that for any table $X\stackrel{f}{\longleftarrow} T\stackrel{g}{\longrightarrow}Y$ there is exactly one function $\sfromto hTP$ for which $f = p\circ h$ and $g= q\circ h$. 
For a product, the legs $p$ and $q$ are called the \newterm{projections}.
\end{defn}

\begin{principle}[princ:Products]\label{ax:Products}
...
\begin{defn}
	For sets $P$, $X$ and $Y$, a \newterm{parametric function from $X$ to $Y$} is a function $\sfromto g{P\times X}Y$ for some set $P$.
	The set $P$ will be called the \newterm{parameter set}.
	...
	The function $k$ acts like a \newterm{change of parameters} because it transforms the parametric function with parameters in $Q$ into a parametric function with parameters in $P$. 
	Specifically, $f\circ(k\times \id_X)$ is given by the rule $(c,a)\mapsto f(k(c),a)$.
	
	An \newterm{evaluation map} for $X$ and $Y$ is a parametric function $\sfromto a{F\times X}Y$ with parameter set $F$, so that for any 
	parametric function $\sfromto g{P\times X}Y$ there is exactly one change of parameters $\fromto hPF$ so that
	$g = a\circ (h\times \id_X)$. In that case, $F$ is called an \newterm{exponential with base $Y$ and exponent $X$}.
\end{defn}

\begin{principle}[Function Sets Exist]\label{princ:Exponents}
...
%\begin{exercises}
%	\begin{enumerate}
%		\item Explain how $\One\stackrel{\hat{\tt}}{\longrightarrow}\Two$ is universal. [There is not a single intended correct answer to this. 
...
Now that we have sets and functions we can consider a function $\fromto a\NN A$ to be an \newterm{infinite sequence}: $a(0)$, $a(1)$, $a(2)$, \ldots.
...
\begin{defn}
	A \newterm{sequence in set $X$} is a function $\fromto a\NN X$.
\end{defn}
...
\begin{defn}\label{def:nno}
	A \newterm{simple recurrence} is a set with functions $\One\stackrel{\hat{b}}{\longrightarrow}X\stackrel{r}{\longleftarrow}X$. We will say the two functions $\hat{b}$ and $r$ form a \newterm{recurrence on $X$}.
	
	A \newterm{natural number set} is a simple recurrence   $\One\stackrel{\hat{z}}{\longrightarrow}N\stackrel{s}{\longleftarrow}N$ so that for any other simple recurrence $\One\stackrel{\hat{b}}{\longrightarrow}X\stackrel{r}{\longleftarrow}X$, there is exactly one function $\sfromto fNX$ so that $f\circ \hat{z} =\hat b$ and $f\circ s = r\circ f$.
\end{defn}

The principle we are interested in here is that simple recurrences determine sequences.

\begin{principle}[Natural Numbers for a Set]\label{princ:NNO}
...
\begin{defn}
	A \newterm{primitive recurrence in $A$ (with parameters in $C$)} consists of two  functions $C\stackrel{b}{\longrightarrow} A\stackrel{r}{\longleftarrow}\NN\times C\times A$.
	
	A \newterm{parametric sequence in $A$ with parameters in $C$} is a function
...
\begin{exercise}
	...
	\item Let $\sfromto p\NN\Two$ be a characteristic function. Show that bounded existential quantification is primitive recursive. That is, define the function $\fromto {\exists^<_p}\NN\Two$ by $\exists^p_<(n) = \tt$ iff there is at least one $k<n$ for which $p(k)=\tt$.  Show that this can be defined by a primitive recurrence.
	[Hint: $\exists^p_<(0) = \ff$ because there are no natural numbers below $0$. 
...
\begin{defn}
	For a set $X$, a \newterm{simple $X$-recurrrence} is a set with two functions
	$\One \stackrel{\hat b}{\longrightarrow} A \stackrel{r}{\longleftarrow} X\times A$.
	
	For a set $X$, an \newterm{$X$-list set} is a simple $X$-recurrence 
...
\begin{principle}[$\List(X)$ is a Set]\label{princ:ListObjects}
...
For this lecture, we concentrate on a fixed set $U$, which we will think of as our ``universe''. For example, if we are currently mainly interested in natural numbers, $U$ could be taken to be $\NN$. If we are interested in poker, it could be $\symb{Deck}$. If we are interested in functions on the reals, it could be $\RR^\RR$. 

The exponential $\Two^U$ is the set of all characteristic maps on $U$.
Each $k\in \Two^U$ corresponds to a subset of $U$ by $k^{-1}(\tt) = \{x\in U\st k(x)=\tt\}$. Vice versa, a subset $A\subseteq U$ determines a characteristic function $\fromto {\kappa_A}U\Two$ by the rule
...
\begin{principle}[Powersets Exist]\label{princ:Powerset}
	For any set $U$, the collection of subsets of $U$ is a set, denoted by $\powerset(U)$ and called the \newterm{power set of $U$}. 
	...
	is given by the rule $p\mapsto \{x\in U\st f(p,x)=\tt\}$.
\end{principle}
...
This must send $k\in \Two^U$ to the subset $k^-(\tt)$.
So it is given by the rule $k\mapsto k^-(\tt)$.
...
This is called the \newterm{intersection} and is denoted by $A\cap B$.
...
Flipping things the other way, there is also a smallest set $C$ so that $A\subseteq B\cup C$. This is called the \newterm{set difference} and is denoted by $A\setminus B$. This is defined by ``but not''. So $x\in A\setminus B$ if and only if $x\in A$ but not $x\in B$. 
...
It turns out that ``$P$ implies False'' is essentially the same as saying ``$P$ is not true''. And if ``$P$ is not true'' is not true, then ``$P$'' must be true. This oservation indicates that the Heyting algebra of subsets is actually a \newterm{Boolean algebra}.
...
\begin{lem}
	...
	\begin{proof}
		...
		So for each $h \in \{\wedge,\vee,\to,-\}$, there  is a subset $(h\circ\langle \kappa_A,\kappa_B\rangle)^-(\tt)$.
...
This lemma, together with the fact that $\emptyset$ is the smallest element of $\powerset(X)$ and $X$ is the largest, can be summarized by saying that $\cap$, $\cup$, $\Rightarrow$, $\emptyset$ and $X$ make $\powerset(X)$ into what is known as a \newterm{Heyting algebra}. 
...
\begin{itemize}
	...
	In fact, any binary operation that is associative, commutative and idempotent is called a \newterm{semilattice operation}.
...
\begin{defn}
	A \newterm{relation between $X$ and $Y$} is a subset $R\subseteq{X\times Y}$. 
	We can write $x\mathrel{R} y$ instead of $(x,y)\in R$ to mimic familiar examples.
	
	A \newterm{relation on $X$} is a relation between $X$ and $X$. 
\end{defn}

Here are two common equivalent formulations.
\begin{itemize}
	\item $R\subseteq X\times Y$ determines a characteristic function $\fromto {\kappa_R}{X\times Y}\Two$ so that $R = \{(x,y)\in X\times Y\st \kappa_R(x,y)=\tt\}$.
...
In this sense, $\powerset(U)$ is \newterm{complete}.
That is, the union of any family of subsets of $U$ exists.
This justifes saying that $\powerset(U)$ is a \newterm{complete} Boolean algebra.
...
An \newterm{atom} of a Boolean algebra is an element with the property that there is nothing strictly between the smallest element and it.
...
\begin{lem}
	...
	\begin{proof}
		...
		In other words ${\mathord\ni}_U(s(x),y)=\tt$ if and only if $x=y$.
		Since ${\mathord\ni}_U(s(x),y) =\tt$ if and only if $y\in s(x)$, it is the case that $y\in s(x)$ if and only if $x=y$.
		So $s(x) = \{x\}$. 
	\end{proof}
\end{lem}

Now every subset of $U$ is obtained as a union of singletons: $A = \bigcup_{x\in A}\{x\}$. 
For a complete Boolean algebra, this is what is meant by saying that $\powerset(U)$ is a complete \newterm{atomic} Boolean algebra. 
...
\begin{defn}
	...
	$f^+(A)\subseteq Y$ is called the \newterm{forward image of $A$ with respect to $f$}.
\end{defn}
...
We now can justify how inductive proofs on natural numbers work from a set theoretic perspective.
Consider some $P\subseteq \NN$.
Define $P' = \{n\in\NN\st n^\nxt\in P\}$.
Now suppose $0\in P$ and $k\in P$ implies $k^\nxt\in P$.
Then the simple recurrrence $\One\stackrel{\hat 0}{\longrightarrow}P\stackrel{\succ}{\longleftarrow}P$ determines a function $\fromto{\srec[0,\succ]}\NN P$. This is evidently one-to-one: 
...
\begin{principle}[Unions Exist]\label{princ:Unions}
...
\begin{principle}[Monomorphisms have Characteristic Functions]\label{princ:subobjects}
	For a monomorphism $\sfromto mXY$ there is a unique function $\sfromto {\kappa_m}Y\Two$ so that $m$ is an equalizer for the equation $\kappa_m(y) = \tt$. 
	In other words, $\kappa_m\circ m = \constant_\tt$ and if $\sfromto fWY$ is a function satisfying $\kappa_m\circ f=\constant_\tt$, then there is a unique function $\sfromto {m\backslash f}WX$ so that $f = (m\backslash f)\circ m$.  
	...
	\tt & \text{if $y = m(x)$ for some $x\in X$}\\
	\ff & \text{otherwise}.
	\end{cases}\]
	Suppose $\sfromto fWY$ is a function satisfying $\kappa_m\circ f = \constant_\tt$. Then $(m\backslash f)$ satisfies $(m\backslash f)(w) = x$ if and only if $f(w)=m(x)$. 
\end{principle}

In Figure \ref{fig:covariance-example}, the function $f$ satisfies $\kappa_m\circ f = \constant_\tt$, so there is indeed a unique function $(m\backslash f)$ as indicated in Figure \ref{fig:factoring-through}.
...
\begin{defn}
	For functions $X\stackrel{m}{\longrightarrow}Y\stackrel{f}{\longleftarrow}W$, say that $f$ is \newterm{$m$-coinvariant} if it is that case that for all $Y\doublerightarrow{h}{k}Z$, $h\circ m=k\circ m$ implies $k\circ f = h\circ f$.
\end{defn}

\begin{lem}
	For function $X\stackrel{m}{\longrightarrow}Y\stackrel{f}{\longleftarrow}W$ where $m$ is a monomorphism, $f$ is $m$-coinvariant if and only if $\kappa_m\circ f = \constant_\tt$.
	
	\begin{proof}
		First, note that $\kappa_m\circ f = \constant_\tt$ if and only if
		$\kappa_m(f(w))=\tt$ for every $w\in W$. But $\kappa_m(y)=\tt$ if and only if there is some $x\in X$ so that $y=m(x)$. Therefore, $\kappa_m\circ f=\tt$ if and only if for every $w\in W$ there exists some $x\in X$ for which $f(w)=m(x)$.
		
		Assume $\kappa_m\circ f=\constant_\tt$, and $h\circ m = k\circ m$. For any $w\in W$, there is some $x\in X$ so that $f(w)=m(x)$. Hence $h(f(w)) = h(m(x)) = k(m(x)) = k(f(w))$ for a suitable choice of $x$. 
...
\begin{lem}\label{lem:coinvariant-factoring}
	...
	\begin{proof}
		There is nothing new to prove. Lemma \ref{lem} ensures if $f$ is $m$-coinvariant, then $\kappa_m\circ f = \constant_\tt$. Principle \ref{princ:subobjects} ensures that $m\backslash f$ exists.
	\end{proof}
\end{lem}
...
\begin{lem}\label{lem:invariant-factoring}
...
A function $\fromto fXY$ is invariant with respect to $[-]_E$ if it is the case that $x_0\mathrel{E}x_1$ implies $f(x_0)=f(x_1)$. Any function with this property determines a function from $X/E$ to $Y$ defined by
$[x]_E\mapsto f(x)$. This is defined on all blocks of $X/E$ because each block is $[x]_E$ for some $x$. Moreover, if $[x_0]_E = [x_1]_E$, then $x_0\mathrel{E}x_1$. So $f(x_0)=f(x_1)$. That is, our rule is unambiguous.

\begin{example}
	We might wish to define a `clock'. Recall that $\ZZ$ denotes the set of all integers. On a clock $1$ and $13$ are the same because one hour after midnight and thirteen hours after midnight read the same on the clock. In fact, $1$, $13$, $25$, and so on all read the same. So let is define the relation $\equiv_{12}$ on $\ZZ$ by saying $a\equiv_{12} b$ if and only if for some integer $m$, it is the case that $a + 12\cdot m = b$. In other words, $a$ and $b$ differ by some even multiple of twelve.
	
	Clearly, $a\equiv_{12}a$ is true for all integers $a$ (take $m=0$). And if $a\equiv_{12}b$ and $b\equiv_{12}c$, then $a + 12\cdot m = b$ and $b+12\cdot n = c$ for some $m$ and $n$. Hence $a +12\cdot (m + n) = c$.
	So $a\equiv_{12}c$. And finally, if $a\equiv_{12}b$, then $a+12\cdot m = b$, so $b + 12\cdot -m = a$. So $b\equiv_{12}a$. This shows that $\equiv_{12}$ is an equivalence relation. Now, $\ZZ/\equiv_{12}$ consists of exactly twelve equivalence blocks: $[0]_{\equiv_{12}}$, \ldots, $[11]_{\equiv_{12}}$. Each block corresponds to an hour on the clock dial.
	
	The quotient set $\ZZ/\equiv_{12}$ is usually denoted by $\ZZ_{12}$. Clearly, the same idea works for any positive integer \emph{modulus} $k$ in place of $12$. We investigate this idea in depth in Lecture \ref{lec:ModularArithmetic}, where applications in cryptography arise. 
\end{example}
...
\begin{lem}
	...
	\begin{proof}
		Define $X\uplus Y \subseteq \powerset(X)\times\powerset(Y)$ to consist only of pairs of the form $(\{x\},\emptyset)$ for $x\in X$ and 
...
\begin{lem}
	...
	\begin{proof}
		...
		So $S(Y)$ is an equalizer $S(Y) = \{A\in\powerset(Y)\st \kappa_{S(Y)}(A) = \tt\}$. But in fact, 
		Now consider the function $R[-]$ from $X$ to $\powerset(Y)$ given by $R[x] = \{y\in Y\st x\mathrel{R}y\}$. Because $R$ is
		functional, $\kappa_{S(Y)}(R[x]) = \tt$ for each $x$ in the domain. So $R[-]$ factors through a function from $\fromto fXY$ so that 
		$R[x] = \{f(x)\}$ for all $x\in X$. 
	\end{proof} 
\end{lem}
...
\begin{principle}[Functional Relations Determine Functions]\label{princ:graphs}
...
\begin{exer}
	\begin{exercise}
		...
		\item Show that for any set $X$, $\Gamma_{\id_X} = \Delta_X$. 
	\end{exercise}
\end{exer}

\section{Splitting and the Axiom of Choice}

The us start the section with exercises.

\begin{exer}
Suppose functions $\fromto sYX$ and $\fromto rXY$ satisfy $r\circ s = \id_Y$.
\begin{exercise}
	\item Show that $r$ is an epimorphism.
	\item Show that $s$ is a monomorphism.
	\item Show that $s\circ r$ is \emph{idempotent}: $(s\circ r)\circ(s\circ r)=s\circ r$.
\end{exercise}	
\end{exer}

\begin{defn}
	Two functions $\fromto sYX$ and $\fromto rXY$ satisfying $r\circ s = \id_Y$ are called a \emph{section-retract pair}. An idempotent function $fXX$ ($f\circ f = f$) is called a \emph{retraction}.
\end{defn}

The latest exercise shows that every section-retract pair gives rise to a retraction. Conversely, if $\fromto fXX$ is a retraction, we may define $Y=f^+(X)$  -- the forward image of $X$. Then $f$ restricted to $Y$ is a function $\fromto sYX$ and corestricted to $Y$ is a function $\fromto rXY$. Evidently, $s$ and $r$ form a section-retract pair for which $f=r\circ s$.
So retractions and section-retract pairs are essentially the same things.

In a section-retract pair, the section is a monomorphism. In fact, nearly all monomorphisms are sections.

\begin{lem}
	Suppose $Y\neq\emptyset$. For any monomorphism $\fromto mYX$, there is a function $\fromto rXY$ so that $r\circ m = \id_Y$.
	
	\begin{proof}
		Since $Y\neq \emptyset$, we may pick some $b_0\in Y$. Now define
		a relation $R\subseteq X\times Y$ by $a\mathrel{R}b$ if and only if either $a=m(b)$ or it is the case that $\forall \in Y.a\neq m(y)$ and $b=b_0$. This is a total relation clearly. And it is deterministic because $m$ is one-to-one. Hence $R$ determines a function $r$ from $X$ to $Y$. Apparently, $r(m(b)) = b$ because $m(b)\mathrel{R}b$. 
	\end{proof}
\end{lem}

This tells us that every monomorphism from a non-empty domain is \emph{split}: it is the section of a section-retract pair.

The dual of this would be that every epimorphism is also split. 
But it turns out that the dual is not provable without additional assumptions about $Y$. 
We won't go into what additional assumptions would help in general, but a hint is given by the natural numbers. 
Suppose $\fromto e\NN X$ is an epimorphism. 
Then each subset $e^-(x)\subseteq \NN$ is not empty. Every non-empty subset of $\NN$ has a least element. So we may define a section corresponding to $e$ by setting $s(x) = \text{the least $n\in \NN$ so that $e(n)=x$}$. 
The point is that $\NN$ has enough structure (every non-empty subset has a least element) to allow us to \emph{define} a section.

Without additional structure, it seems reasonable that a section might \emph{exist} for an epimorphism, even though we may not be able to \emph{construct} one explicitly. 
The Axiom of Choice addresses this situation. 
Roughly is says that some how, one can make arbitrarily many choices all at once, without having an explicit construction of those choices.
Though we will not need it very often in this course, the Axiom of Choice (AC) is quite useful in other areas of mathematics, sometimes to prove something that genuinely needs the axiom, other times to simplify a proof that could have been proved without it.

\begin{principle}[Axiom of Choice]\label{princ:choice}
	For every epimorphism $\sfromto eXY$ there is a function $\sfromto cYX$ for which $e\circ c = \id_Y$
\end{principle}
...
Other strong axioms of sets are possible. There are even axioms that inconsistent with AC in the sense that they implies that there actually are epimorphisms that can not be split.
For these lectures, though, our goal is to have machinery to do ``ordinary'' mathematics. The Axiom of Choice falls into that category.
